...
async def generate_response(channel_id, user_message):
    ...
    async with ZepClient(ZEP_API_URL, ZEP_API_KEY) as client:
        try:
            historical_messages = await zep_client.message.aget_session_messages(session_id)
            print(len(historical_messages))
        except Exception as e:
            print(f"Error retrieving historical messages: {e}")
            historical_messages = []

    system_message = {"role": "system", "content": "You are a helpful assistant in a Discord server. Use the provided conversation history to give informed and coherent responses."}
    ...
    try:
        ...
        recent_prompts.append({"question": user_message, "prompt_summary": full_prompt})
        if len(recent_prompts) > 5:
            recent_prompts.pop(0)

        if len(ai_response) > 2000:
            summarize_messages = [
                {"role": "system", "content": "You are a summarizer. Summarize the following text in less than 1500 characters while retaining the most important information."},
